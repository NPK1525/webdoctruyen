...
list.querySelectorAll('[data-report-action]').forEach(button=>button.onclick=async()=>{const result=await apiFetch('/api/reports/'+button.dataset.reportId,{method:'PATCH',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({status:button.dataset.reportAction})});if(result.ok)load();});
  }
